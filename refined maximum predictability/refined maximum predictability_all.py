...
for ij in ff.readlines():
    ...
    for j in range(1,len(dseq11)):
        ...
        if j>=begain:
            ...
            refined_maximum_predictability=max([float(r_tod),float(r_td),float(r_od),
                                                float(r_d),float(r_tod2),float(r_td2)])
            #####################3
            set77=mess[0:-2]+([str(j+1),r_tod,r_td,r_od,r_d,r_tod2,r_td2,refined_maximum_predictability])
